...
function temperatureInF(temperature, unit) {
  if (unit === "f") {
    ...
    return "You did not input a valid unit";
  }
  /* code here */
}
...
function isItAnApple(array) {
  let applearray = array;
  let applearraylength = applearray.length;
  let newarray = [];
  for (let i = 0; i < applearraylength; i++) {
    let current = array[i];
    if (current === "apple") {
      newarray.push("true");
    } else {
...
function getModelYears(inventory) {
  let invarraylength = inventory.length;
